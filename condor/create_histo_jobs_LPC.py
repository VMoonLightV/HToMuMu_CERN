...
send_all_jobs = open(CONDOR_BASE_DIR + "/condor_histo_job_sender.sh", "w+")
...
for dataset_name in list_datasets:
    ...
    file_copy_name = channel + "_" + era + "_histos_v1.root"
    ...
    JOB_DIR = CONDOR_BASE_DIR + "histos/" + "%s/"%(dataset_name)
    ...
    ###################################################
    os.system("cp " + "%s/template_create_histos_job_LPC.sh"%(CONDOR_BASE_DIR) + " " + "%s/run_job_LPC.sh"%(JOB_DIR))
    os.system("cp " + "%s/bin/CreateHistograms"%(ANALYZER_DIR) + " " + JOB_DIR)
    ...
    transfer_files += JOB_DIR + "/CreateHistograms, "
