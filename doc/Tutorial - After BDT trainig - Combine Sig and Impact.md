### Statement
Author: Yuyang
This tutorial is based on [Matias' tutorial of flashggFinalFit](https://github.com/MatBarria/flashggFinalFit) for yuyang, which is used for Run3 datacard analysis in CMSSW_14_0_14. However, that environment can't be used for Run2 datacard analysis, thus we build this version to combine Run2 and Run3 datacards. The new branch is in [yuyang's flashggFinalFit repo](https://github.com/VMoonLightV/flashggFinalFit) currently, maybe you can move it to organization in the future.
...
Therefore, we turn to find a version of CMSSW where we can run both Run2 and Run3 datacards, which is done successfully by others. After trying different versions of CMSSW, now we can run both Run2 and Run3 in CMSSW_11_3_4. You can set up with the following commands:
...
This command will generate Impact plot for all category in the bash script one by one. Ideally, it will copy all pdf plot into Plots/ after it finish.
