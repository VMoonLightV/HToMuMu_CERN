...
#include <TTree.h>
#include <iostream>
// g++ -o ./bin/Split_tuples_region src/Split_tuples_region.cc $(root-config --cflags --libs)
...
int main(int argc, char *argv[]) {
    ...
    TFile output_file(output +"/SR/" + channel + "_" + era + "_skim.root",
                      "RECREATE");
    TTree *tree_output = tree_input->CloneTree(0); // Clone the structure only

    tree_output = tree_input->CopyTree("(diMuon_mass > 110) && (diMuon_mass < 150)");
