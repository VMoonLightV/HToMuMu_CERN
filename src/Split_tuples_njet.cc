...
#include <TTree.h>
#include <iostream>
// g++ -o ./bin/Split_tuples_njet src/Split_tuples_njet.cc $(root-config --cflags --libs)
...
int main(int argc, char *argv[]) {
    ...
    TFile output_file("/eos/home-y/yulou/Fnal-hmm/hmm-tuples/njet/"+njet+"jet/"+ region +"/" + channel + "_" + era + "_skim.root",  "RECREATE");
    TTree *tree_output = tree_input->CloneTree(0); // Clone the structure only

    TString condition;
    if (region == "ZCR") {
        if (njet != "2") { 
                condition = "(n_jet == " + njet + ") && (diMuon_mass > 70) && (diMuon_mass <= 110)"; 
            }
            else if (njet == "2") {
                condition = "(n_jet >= " + njet + ") && (diMuon_mass > 70) && (diMuon_mass <= 110)";
            }
        ...
        if (njet != "2") { 
                condition = "(n_jet == " + njet + ") && (diMuon_mass > 110) && (diMuon_mass <= 150)"; 
            }
            else if (njet == "2") {
                condition = "(n_jet >= " + njet + ") && (diMuon_mass > 110) && (diMuon_mass <= 150)";
            }
        ...
        std::cerr << "No valid region (ZCR or SR): " << region
                  << std::endl;
        return -1;
    }
    tree_output = tree_input->CopyTree(condition);
    //tree_output = tree_input->CopyTree("(n_jet >= 2) && (diMuon_mass < 110)"); //ZCR 
    //tree_output = tree_input->CopyTree("(n_jet >= 2) && (diMuon_mass >= 110)"); //SR
    ...
    inputFile.Close();
    std::cout   << "File in: " 
                << "/eos/home-y/yulou/Fnal-hmm/hmm-tuples/njet/"+njet+"jet/"+ region +"/" + channel + "_" + era + "_skim.root" 
                << std::endl;
}
