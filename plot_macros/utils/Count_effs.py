...
def draw_data_and_simul_and_ratio(
    ...
    eff_dir = "/eos/home-y/yulou/Fnal-hmm/hmm-tuples/njet/nobin_jet/" + region + "_eff/"
    bkg_histograms_list_eff, bkg_bins_list_eff, DY_count_eff = get_histograms_from_tuple(
        eff_dir, background_sources, simulation_era, variables, True, use_puweight,
    ...
    if variable == "diMuon_mass" or variable == "diMuon_bsConstrainedMass":
        bkg_histograms_list_eff[bkg_histograms_list_eff == 0] = -100.0
        # print("Era: " + era + " | Events: " + str(len(branches[variable])))

        
    
    # without muon eff
    noeff_dir = "/eos/home-y/yulou/Fnal-hmm/hmm-tuples/njet/nobin_jet/" + region + "/"
    bkg_histograms_list_noeff, bkg_bins_list_noeff, DY_count_noeff = get_histograms_from_tuple(
        noeff_dir, background_sources, simulation_era, variables, True, use_puweight,
        use_ggH_category, use_VBF_category, njet, region, era == "2024", isZRange, 
    )
    ...
    tot_bg_numpy_hist_eff = np.array([])
    for i, bg_hist in enumerate(bkg_histograms_list_eff):
        if i == 0:
            tot_bg_numpy_hist_eff = bg_hist            
        else:
            tot_bg_numpy_hist_eff = tot_bg_numpy_hist_eff + bg_hist    
    sum_MC_eff = sum([hist.sum() for hist in tot_bg_numpy_hist_eff])

    tot_bg_numpy_hist_noeff = np.array([])
    for i, bg_hist in enumerate(bkg_histograms_list_noeff):
        if i == 0:
            tot_bg_numpy_hist_noeff = bg_hist            
        else:
            tot_bg_numpy_hist_noeff = tot_bg_numpy_hist_noeff + bg_hist     

    ratio_hist, ratio_error = get_histograms_ratio(tot_bg_numpy_hist_eff, tot_bg_numpy_hist_noeff)
    
    #print(tot_bg_numpy_hist_noeff)
    sum_MC_noeff = sum([hist.sum() for hist in tot_bg_numpy_hist_noeff])

    print(DY_count_eff)
    print(sum_MC_eff)
    print(sum_MC_noeff)
    #print(bkg_histograms_list_eff)
    #print("add: ", tot_bg_numpy_hist_eff)

    fig, axs = get_canvas(True)

    bkg_bins_list_eff = np.linspace(x_range[variable_bin][0],x_range[variable_bin][1], n_bins[variable_bin]+1)
    hep.histplot(
        bkg_histograms_list_noeff,
        bkg_bins_list_eff,
        yerr=True,
        histtype="fill",
    ...
    axs[0].set_ylim(0.1, 1000 * np.max(tot_bg_numpy_hist_eff))
    axs[0].set_xlim(bkg_bins_list_eff[0], bkg_bins_list_eff[-1])
    ...
    if "ZCR" in region:
        print(f"{era} {njet}jet {region} eff/noeff MC num_events ({variable}): {sum_MC_eff} / {sum_MC_noeff}") #= {sum_MC_eff/sum_MC_noeff}")
        ...
        if (era=="2022" and (njet=="0" or njet=="nobin_") and variable == "diMuon_pt"):
            with open(csv_path, 'w', newline='') as csvfile:
                ...
                csv_writer.writerow(["Era","Njet",  "Variable", "Region", "eff_MC_Events", "noeff_MC_Events", "DY_count", "Ratio", 
                                        "no_DY_bkg", "Right_DY", "DY_factor"])
                
                csv_writer.writerow([era, njet, variable, region, sum_MC_eff, sum_MC_noeff, DY_count_noeff, sum_MC_eff/sum_MC_noeff, 
                                    sum_MC_noeff - DY_count_noeff, sum_MC_eff - sum_MC_noeff + DY_count_noeff, (sum_MC_eff - sum_MC_noeff + DY_count_noeff)/DY_count_noeff])
    ...
    if(era == "2024"):
        axs[1].set_ylabel("eff_MC/noeff_MC(2023BPix)", loc="center")
    else:
        axs[1].set_ylabel("eff_MC/noeff_MC", loc="center")
    axs[1].set_ylim(0.95, 1.05)
    axs[1].set_xlim(bkg_bins_list_eff[0], bkg_bins_list_eff[-1])
    ...
    output_directory = f"../plots/ratio/njet/{njet}jet_{region}_eff_VS_noeff/" + era + "/"
    if not use_puweight:
    ...
    if(isZRange):
        save_figure(fig, output_directory, variable + "_Z_" + era + "_eff_ratio")
    else:
        save_figure(fig, output_directory, variable + "_" + era + "_eff_ratio")
    plt.close()
