...
def plot_diMuon_comp_and_fit(fit_range, mass_range, noBSC_hist, BSC_hist, particle, era, channel,
                    region_1, region_2, muon1_cut1, muon1_cut2, signal=None):
    ...
    ############# to avoid failing fitting
    x_data = BSC_hist[1][minBin:maxBin]
    y_data = BSC_hist[0][minBin:maxBin]
    mean_guess = np.average(x_data, weights=y_data) if np.sum(y_data) > 0 else masses[particle]
    variance = np.average((x_data - masses[particle])**2, weights=y_data) if np.sum(y_data) > 0 else 1.0
    sigma_guess = np.sqrt(variance)
    #############
    
    popt, pcov = curve_fit(gaussian, noBSC_hist[1][minBin:maxBin], noBSC_hist[0][minBin:maxBin], p0=[np.max(noBSC_hist[0][minBin:maxBin]), masses[particle], 1], maxfev=5000)
    ...
    #if (muon1_cut1 == 26) and (region_1 == "E") and (region_2 == "E"):
    ...
    axs[1].set_xlabel(f"{region_1}{region_2}_mu1_pt_{muon1_cut1}_{muon1_cut2}  " + "m$_{\mu\mu}$ [GeV]")
    ...
    if(particle == "H"): 
        ...
        save_figure(fig, output_directory, particle + "_" + f"{region_1}{region_2}_mu1_pt_{muon1_cut1}_{muon1_cut2}" + "_reso")
    plt.close()

    return poptBSC[2], poptBSC[1], BSC_res, BSC_res_err


def draw_diMuon_mass_peak_EVE(particle, era, channel, use_puweight=True):
    ...
    if particle == "Z":
        ...
        for i in range(len(muon1_pt_cuts) - 1):
            for region_1, eta_range_1 in eta_cuts.items():
                for region_2, eta_range_2 in eta_cuts.items():
                    ...
                    with ur.open("/eos/home-y/yulou/Fnal-hmm/hmm-tuples/EVE_pt_eta/ZCR_75-105/"+ channel+"_" + era + "_skim.root:" + tree_name) as data_file:
                    ...
                    BSC_mass_sigma, BSC_mass_mu, BSC_res, BSC_res_err = plot_diMuon_comp_and_fit(fit_range, mass_range, histograms_list[0], histograms_list[1], 
                                    particle, era, channel,
                                    region_1, region_2, muon1_cut1, muon1_cut2)
        ...
        filename = f"../plots/EVE_resolution/{channel}/{era}/BSC_Z_mass_reso_results.csv"
    ...
    if particle == "H":
        ...
        for signal in signal_sources:
            histograms_list = []
            #labels = []
            with ur.open("/eos/home-y/yulou/Fnal-hmm/hmm-tuples/EVE_pt_eta/ZCR_75-105/" + signal + "_" + era + "_skim.root:tree_output") as data_file:
                branches = data_file.arrays(variables, library="np")
                for var in variables:
                    histogram, bins = np.histogram(
                    branches[var],
