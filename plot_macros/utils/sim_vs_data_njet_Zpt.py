...
if bin600 == True:
    bins_0_200 = np.linspace(0, 200, 41)
...
def get_histograms_from_tuple(
    ...
    for source in sources:
        with ur.open(input_dir + source + "_" + era + "_skim.root:tree_output") as file:
            branches = file.arrays(variables, library="np")

            if cali_draw == True:
                bool_list = (
                    (branches["diMuon_bsConstrainedMass"] > 120)
                    & (branches["diMuon_bsConstrainedMass"] < 130)
                    # & (branches["calibrated_diMuon_bsConstrainedMass_error"] > 0.0)
                    # & (branches["calibrated_diMuon_bsConstrainedMass_error"] < 0.015)
                )
            ...
            # if (variables[0] == "diMuon_bsConstrainedPt"): base_bins[-1] = 10000
            if variables[0] == "diMuon_bsConstrainedPt":
...
def piecewise_polyfit(df, output_name):
    ...
    for i, order_i in zip(range(len(boundaries) - 1), orders):
        ...
        if len(x_segment) < order_i:
            print(
                f"warning: [{boundaries[i]:.2f}, {boundaries[i+1]:.2f}] don't have enough data point."
            )
            continue
        ...
        if (i == 0) & (use_F_test == True):
            ...
            for order_l in range(1, 9):
                for j in range(1, 2):
                    order_h = order_l + j
                    F_val, p_val = f_test_polyfit(
                        x_segment, y_segment, order_l, order_h
                    )
                    print(
                        f"p-value = {p_val:.4f}, F-value = {F_val:.4f}, from order {order_l} to {order_h}"
                    )
...
def draw_data_and_simul_and_ratio(
    ...
    if variable != "diMuon_bsConstrainedMass":
        variables.append("diMuon_bsConstrainedMass")
    ...
    with ur.open(input_dir + "Data_" + era + "_skim.root:tree_output") as data_file:
        ...
        if variable == "diMuon_bsConstrainedPt":
            base_bins = dimuon_pt_bins
    ...
    if "ZCR" in region:
        print(
            f"{era} {njet}jet {region} data/MC num_events ({variable}): {sum_data} / {sum_MC}"
        )  # = {sum_data/sum_MC}")

        if njet == "nobin_":
            csv_path = f"/afs/cern.ch/user/y/yulou/CMSSW_14_0_14/src/HToMuMu/scripts/event_counts_{region}_nobin.csv"
        else:
            csv_path = f"/afs/cern.ch/user/y/yulou/CMSSW_14_0_14/src/HToMuMu/scripts/event_counts_{region}_bin.csv"

        if (
            era == "2022"
            and (njet == "0" or njet == "nobin_")
            and variable == "diMuon_bsConstrainedPt"
        ):
            with open(csv_path, "w", newline="") as csvfile:
                ...
                csv_writer.writerow(
                    [
                        ...
                        sum_data / sum_MC,
                        sum_MC - DY_count,
                        sum_data - sum_MC + DY_count,
                        (sum_data - sum_MC + DY_count) / DY_count,
                    ]
                )
    ...
    # if (variable == "diMuon_bsConstrainedPt"): axs[1].set_xlim(x_range[variable][0], x_range[variable][1])
    if variable == "diMuon_bsConstrainedPt":
        if bin600 == False:
    ...
    if ("ZCR_normalization" in region) & (variable == "diMuon_bsConstrainedPt"):
        ...
        iter_dir = f"../plots/ratio/njet_9-19_nobin/{njet}jet_ratio_table_dimuon_pt_{region}/"
        if bin600 == True:
            iter_dir = (
                f"../plots/ratio/njet_9-19_nobin/{njet}jet_ratio_table_dimuon_pt_{region}_0-600/"
            )
    ...
    output_directory = f"../plots/ratio/njet_9-19_nobin/{njet}jet_{region}/" + era + "/"
    if bin600 == False:
        output_directory = f"../plots/ratio/njet_9-19_nobin/{njet}jet_{region}_0-200/" + era + "/"
    else:
        output_directory = f"../plots/ratio/njet_9-19_nobin/{njet}jet_{region}_0-600/" + era + "/"
    if not use_puweight:
